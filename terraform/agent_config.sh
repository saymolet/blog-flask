...
passwd_config(){
  ...
  usermod -aG docker agent && usermod -s /bin/bash agent
}
...
install_tools(){
  ...
  tar -xvf /tmp/helmfile.tar.gz -C /tmp/ && mv /tmp/helmfile /usr/bin/ && pip install poetry && pip install --upgrade requests
}
